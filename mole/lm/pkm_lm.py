...
from mole.lm.pkm import PKM

class pkmlm_config(PretrainedConfig):
    model_type = "PKMLM"
...
class PKMLM(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        if not config.offload_tok_embbedings:
            self.tok_embeddings = nn.ModuleList([nn.Embedding(config.vocab_size, config.dim)])
            nn.init.normal_(self.tok_embeddings[0].weight, std=config.embedding_init_std)
        else:
            # 防止被注册
            self.tok_embeddings = [SparseEmbedding(
    ...
    def forward(self,
        ...
        h = self.tok_embeddings[0](input_ids)
        pos_cis = self.pos_cis[start_pos:start_pos + input_ids.size(1)]
        past_kvs = []
        states = {}
        for l, layer in enumerate(self.layers):
            h, past_kv, states[l] = layer(
                h, pos_cis,
            ...
            if l in self.pkm_dict:
                x, pkm_layer = self.pkm_dict[l]
                h = h + pkm_layer(states[x])
...
if __name__ == '__main__':
    import json
    with open('/NAS/wujunkang/guizhiyu/mole/config/pkm14m.json', 'r') as f:
        cfg = json.load(f)
        cfg = pkmlm_config(**cfg)
    model = PKMLM(cfg)
    model.to('cuda:0')
